...
export default function App(){
 ...
 const play=useSound(sound),suspense=useSuspenseMusic(),bomb=bombs[bombIndex],team=teams[turnIndex%Math.max(teams.length,1)]
 ...
 useEffect(()=>{if(screen==='game'&&sound)suspense.start();else suspense.stop();return()=>{if(screen!=='game')suspense.stop()}},[screen,sound])
 ...
 const begin=()=>{const ready=names.map((n,i)=>({id:crypto.randomUUID(),name:n.trim(),color:teamColors[i],score:0})).filter(t=>t.name);if(ready.length<2||!coverage[4]||!coverage[7]||!coverage[10]||(questionMode==='manual'&&!manualReady))return;const first=questionFor(0,[]);setTeams(ready);setTurnIndex(0);setBombIndex(0);setUsed(first?[first.id]:[]);setQuestion(first);setSeconds(timeFor(0));setStatus('armed');setSelected(null);setCutWires([]);setScreen('game');play('start');suspense.start()}
 const advance=()=>{if(bombIndex===6){suspense.stop();play('win');setScreen('result');return}const next=bombIndex+1,q=questionFor(next,used);setBombIndex(next);setQuestion(q);setUsed(v=>q?[...v,q.id]:v);setSeconds(timeFor(next));setSelected(null);setCutWires([]);setStatus('armed');play('start')}
