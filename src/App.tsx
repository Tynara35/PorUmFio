...
const choose=(i:number)=>{if(status!=='armed'||!question||cutWires.includes(i))return;setSelected(i);setStatus('cutting');play('cut');setTimeout(()=>{const cutCorrect=question.options?.[i]===question.answer;if(cutCorrect){const remaining=bomb.wires-cutWires.length;if(remaining===2&&teams.length>1){const next=teams[(turnIndex+1)%teams.length];setTeams(v=>v.map(t=>t.id===next.id?{...t,score:t.score+pointsFor(bombIndex)}:t))}rotateTeam();setStatus('exploded');play('boom');setTimeout(advance,1900);return}const nextCuts=[...cutWires,i];setCutWires(nextCuts);if(nextCuts.length===bomb.wires-1){setSelected(null);setStatus('safe');setTeams(v=>v.map(t=>t.id===team.id?{...t,score:t.score+pointsFor(bombIndex)}:t));rotateTeam();play('safe');setTimeout(advance,1900)}else{rotateTeam();setTimeout(()=>{setSelected(null);setStatus('armed')},450)}},600)}
